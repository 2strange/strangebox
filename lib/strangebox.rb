...
class Strangebox < Sinatra::Base
  ...
  autoload :GemVersionCollection, "strangebox/gem_version_collection"
  ...
  post '/upload' do
    return "StrangeBox:: Please ensure #{File.expand_path(Strangebox.data)} is writable by the strangebox web server." unless File.writable? Strangebox.data
    unless params[:file] && (tmpfile = params[:file][:tempfile]) && (name = params[:file][:filename])
      @error = "StrangeBox:: No file selected"
      return erb(:upload)
    end
    ...
    if Strangebox.disallow_replace? and File.exist?(dest_filename)
      ...
      if existing_file_digest != tmpfile_digest
        return error_response(409, "StrangeBox:: Gem already exists, you must delete the existing version first.")
      else
        return [200, "StrangeBox:: Ignoring upload, you uploaded the same thing previously."]
      end
    end
